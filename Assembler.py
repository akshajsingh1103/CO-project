...
operations = {
    ...
    "BONUS": {
        "mul": {"opcode": "0000000", "funct3": "000"},
        "rst": {"opcode": "0000000", "funct3": "001"},
        "halt": {"opcode": "0000000", "funct3": "010"},
        "rvrs": {"opcode": "0000000", "funct3": "011"},
    }
}
...
def assemble_j_type(instruction):
    rd = register_encoding[instruction["rd"]]
    imm = get_binary(int(instruction["imm"]), 21)
    return f"{imm[0]}{imm[10:20]}{imm[9]}{imm[1:9]}{rd}{instruction['opcode']}"

# BONUS PART

def assemble_mul(instr):
    rs1 = register_encoding[instr["rs1"]]
    rs2 = register_encoding[instr["rs2"]]
    rd = register_encoding[instr["rd"]]
    return f"{instr['funct7']}{rs2}{rs1}{instr['funct3']}{rd}{instr['opcode']}"


def assemble_bonus(instr):
    imm = "0" * 17
    rd = "0" * 5
    return f"{imm}{instr['funct3']}{rd}{instr['opcode']}"


def assemble_rvrs(instr):
    imm = "0" * 12
    rs1 = register_encoding[instr["rs1"]]
    rd = register_encoding[instr["rd"]]
    return f"{imm}{rs1}{instr['funct3']}{rd}{instr['opcode']}"
...
for line in file:
    ...
    # Bonus
    elif re.search("^mul [^ ]{2,4},[^ ]{2,4},[^ ]{2,4}.*", line):
        operation = "mul"
        ...
        binary_list.append(assemble_mul(instruction))

    elif re.search("^rst", line):
        operation = "rst"
        ...
        binary_list.append(assemble_bonus(instruction))
    ...
    elif re.search("^rvrs [^ ]{2,4},[^ ]{2,4}.*", line):
        operation = "rvrs"
        ...
        instruction = {
            ...
            "opcode": "0000000",
            "rd": line[line.find(" ") + 1 : line.find(",")],
            "rs1": line[line.find(",") + 1 :],
            "funct3": "011",
            "funct7": "0000000",
        }
        binary_list.append(assemble_rvrs(instruction))
